...
static NOINLINE void course_preaim_step_to_target(uint16_t target_x)
{
...
static NOINLINE void course_preaim_update_when_unlocked(void)
{
    uint16_t target_x;
    ...
    target_x = course_preaim_target_x_us();

    if (g_aim_confident_target == 0U) {
        course_preaim_step_to_target(target_x);
