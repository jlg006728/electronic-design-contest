/*
 * V70 MSPM0G3507 + turn-angle/encoder calibration logger.
 *
 * Wiring:
 ...
 *
 * Scope:
 *   The grayscale array, TB6612 motors, MPU gyro and encoder logger are active.
 *   OpenMV, servos, encoder motor control and mission logic remain disabled.
...
#define PWM_RIGHT_BASE_TICKS             429U
#define PWM_MIN_TICKS                    250U
#define PWM_MAX_TICKS                    657U
#define START_BOOST_LEFT_TICKS           563U
...
#define TURN_LOG_CAPACITY                16U
#define TURN_LOG_REENTRY_CONFIRM_FRAMES  2U
...
#define LEFT_ENCODER_A_PIN               DL_GPIO_PIN_12
#define LEFT_ENCODER_B_PIN               DL_GPIO_PIN_13
#define RIGHT_ENCODER_A_PIN              DL_GPIO_PIN_14
#define RIGHT_ENCODER_B_PIN              DL_GPIO_PIN_15
#define ENCODER_INTERRUPT_MASK           (LEFT_ENCODER_A_PIN | RIGHT_ENCODER_A_PIN)
...
volatile int32_t g_left_forward_total = 0;
volatile int32_t g_right_forward_total = 0;

volatile uint8_t g_turn_log_count = 0U;
volatile int8_t g_turn_log_active_index = -1;
volatile uint8_t g_turn_log_waiting_for_line = 1U;
volatile uint8_t g_turn_log_reentry_count = 0U;
volatile uint8_t g_turn_log_overflow = 0U;
volatile uint8_t g_turn_log_status[TURN_LOG_CAPACITY] = {0U};
volatile int8_t g_turn_log_direction[TURN_LOG_CAPACITY] = {0};
volatile uint8_t g_turn_log_false_candidate_count[TURN_LOG_CAPACITY] = {0U};
volatile uint32_t g_turn_log_entry_frame[TURN_LOG_CAPACITY] = {0U};
volatile uint32_t g_turn_log_first_loss_frame[TURN_LOG_CAPACITY] = {0U};
volatile uint32_t g_turn_log_confirm_frame[TURN_LOG_CAPACITY] = {0U};
volatile int32_t g_turn_log_entry_yaw_mdeg[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_first_loss_yaw_mdeg[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_confirm_yaw_mdeg[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_first_loss_angle_mdeg[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_confirm_angle_mdeg[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_entry_left_count[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_entry_right_count[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_first_loss_left_delta[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_first_loss_right_delta[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_confirm_left_delta[TURN_LOG_CAPACITY] = {0};
volatile int32_t g_turn_log_confirm_right_delta[TURN_LOG_CAPACITY] = {0};
...
{
    init_input(IOMUX_PINCM29); /* PB12: left encoder A */
    init_input(IOMUX_PINCM30); /* PB13: left encoder B */
    init_input(IOMUX_PINCM31); /* PB14: right encoder A */
    init_input(IOMUX_PINCM32); /* PB15: right encoder B */

    DL_GPIO_setLowerPinsPolarity(ENCODER_PORT,
        DL_GPIO_PIN_12_EDGE_RISE_FALL |
        DL_GPIO_PIN_14_EDGE_RISE_FALL);
    DL_GPIO_clearInterruptStatus(ENCODER_PORT, ENCODER_INTERRUPT_MASK);
    DL_GPIO_enableInterrupt(ENCODER_PORT, ENCODER_INTERRUPT_MASK);
    NVIC_ClearPendingIRQ(GPIOB_INT_IRQn);
    NVIC_EnableIRQ(GPIOB_INT_IRQn);
}

static NOINLINE void encoder_update_forward_totals(void)
{
    int32_t left_count;
    int32_t right_count;

    __disable_irq();
    left_count = g_left_encoder_count;
    right_count = g_right_encoder_count;
    __enable_irq();

    /* Confirmed convention: left forward is negative, right forward positive. */
    g_left_forward_total = -left_count;
    g_right_forward_total = right_count;
}
...
static NOINLINE void encoder_reset_counts(void)
{
    ...
    g_left_forward_total = 0;
    g_right_forward_total = 0;
}

static NOINLINE void turn_log_begin(void)
{
    uint8_t index;

    if (g_turn_log_count >= TURN_LOG_CAPACITY) {
        g_turn_log_overflow = 1U;
        g_turn_log_active_index = -1;
        g_turn_log_waiting_for_line = 0U;
        return;
    }

    index = g_turn_log_count;
    g_turn_log_count++;
    g_turn_log_active_index = (int8_t) index;
    g_turn_log_waiting_for_line = 0U;
    g_turn_log_reentry_count = 0U;
    g_turn_log_status[index] = 1U;
    g_turn_log_direction[index] = 0;
    g_turn_log_false_candidate_count[index] = 0U;
    g_turn_log_entry_frame[index] = g_run_frame;
    g_turn_log_entry_yaw_mdeg[index] = g_yaw_mdeg;
    g_turn_log_entry_left_count[index] = g_left_forward_total;
    g_turn_log_entry_right_count[index] = g_right_forward_total;
}

static NOINLINE void turn_log_update_reentry(void)
{
    if (g_turn_log_waiting_for_line == 0U) {
        return;
    }

    if (g_gray_line_valid != 0U) {
        if (g_turn_log_reentry_count < TURN_LOG_REENTRY_CONFIRM_FRAMES) {
            g_turn_log_reentry_count++;
        }
    } else {
        g_turn_log_reentry_count = 0U;
    }
...
static NOINLINE void turn_log_capture_first_loss(int8_t direction)
{
    uint8_t index;

    if (g_turn_log_active_index < 0) {
        return;
    }

    index = (uint8_t) g_turn_log_active_index;
    g_turn_log_status[index] = 2U;
    g_turn_log_direction[index] = direction;
    g_turn_log_first_loss_frame[index] = g_run_frame;
    g_turn_log_first_loss_yaw_mdeg[index] = g_yaw_mdeg;
    g_turn_log_first_loss_angle_mdeg[index] =
        g_yaw_mdeg - g_turn_log_entry_yaw_mdeg[index];
    g_turn_log_first_loss_left_delta[index] =
        g_left_forward_total - g_turn_log_entry_left_count[index];
    g_turn_log_first_loss_right_delta[index] =
        g_right_forward_total - g_turn_log_entry_right_count[index];
}

static NOINLINE void turn_log_cancel_candidate(void)
{
    uint8_t index;
    ...
    index = (uint8_t) g_turn_log_active_index;
    if (g_turn_log_status[index] == 2U) {
        if (g_turn_log_false_candidate_count[index] < 255U) {
            g_turn_log_false_candidate_count[index]++;
        }
        g_turn_log_status[index] = 1U;
    }
}

static NOINLINE void turn_log_confirm_exit(void)
{
    uint8_t index;
    ...
    index = (uint8_t) g_turn_log_active_index;
    g_turn_log_status[index] = 3U;
    g_turn_log_confirm_frame[index] = g_run_frame;
    g_turn_log_confirm_yaw_mdeg[index] = g_yaw_mdeg;
    g_turn_log_confirm_angle_mdeg[index] =
        g_yaw_mdeg - g_turn_log_entry_yaw_mdeg[index];
    g_turn_log_confirm_left_delta[index] =
        g_left_forward_total - g_turn_log_entry_left_count[index];
    g_turn_log_confirm_right_delta[index] =
        g_right_forward_total - g_turn_log_entry_right_count[index];
    g_turn_log_active_index = -1;
    g_turn_log_waiting_for_line = 1U;
    g_turn_log_reentry_count = 0U;
}
...
int main(void)
{
    ...
    for (g_run_frame = 0U; g_run_frame < RUN_MAX_FRAMES; g_run_frame++) {
        mpu_update_sample();
        encoder_update_forward_totals();
        gray_scan_all_channels();
        gray_update_baseline_and_changes();
        turn_log_update_reentry();
...
void GROUP1_IRQHandler(void)
{
    ...
    if ((status & LEFT_ENCODER_A_PIN) != 0U) {
        uint32_t pins = DL_GPIO_readPins(
            ENCODER_PORT, LEFT_ENCODER_A_PIN | LEFT_ENCODER_B_PIN);
        bool a_high = ((pins & LEFT_ENCODER_A_PIN) != 0U);
        bool b_high = ((pins & LEFT_ENCODER_B_PIN) != 0U);
    ...
    if ((status & RIGHT_ENCODER_A_PIN) != 0U) {
        uint32_t pins = DL_GPIO_readPins(
            ENCODER_PORT, RIGHT_ENCODER_A_PIN | RIGHT_ENCODER_B_PIN);
        bool a_high = ((pins & RIGHT_ENCODER_A_PIN) != 0U);
        bool b_high = ((pins & RIGHT_ENCODER_B_PIN) != 0U);
