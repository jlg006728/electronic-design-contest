...
#define PHASE_START_GAP_ALIGN       9U
...
volatile uint8_t g_start_gap_align_frames_remaining = 0;
...
static NOINLINE void start_gap_bridge_update(void)
{
    ...
    if (g_line_valid != 0U) {
        ...
        if (g_start_gap_reacquire_count >= START_GAP_REACQUIRE_FRAMES) {
            ...
            g_start_gap_bridge_frames = 0U;
            g_start_gap_reacquire_count = 0U;
            g_start_gap_align_frames_remaining = START_GAP_ALIGN_FRAMES;
            g_start_gap_reacquired_event_count++;
            g_line_lost_count = 0U;
            line_follow_update();
            g_phase = PHASE_START_GAP_ALIGN;
...
static NOINLINE void start_gap_align_update(void)
{
    read_line_sensors();
    ...
    if (g_start_gap_align_frames_remaining > 0U) {
...
int main(void)
{
    ...
    while (1) {
        ...
        if (g_phase == PHASE_START_GAP_ALIGN) {
            start_gap_align_update();
            continue;
        }
