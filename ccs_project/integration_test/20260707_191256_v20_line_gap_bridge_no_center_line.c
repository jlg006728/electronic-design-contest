...
#define LINE_GAP_ENTER_ERROR        2400
#define LINE_GAP_MAX_SAMPLES        180U
#define LINE_GAP_LEFT_TICKS         PWM_LEFT_BASE_TICKS
#define LINE_GAP_RIGHT_TICKS        PWM_RIGHT_BASE_TICKS
...
volatile uint32_t g_line_gap_bridge_count = 0;
...
static NOINLINE bool line_gap_bridge_handle(void)
{
    ...
    if (g_line_valid != 0U) {
        ...
        g_line_gap_bridge_count = 0U;
        return false;
    }

    if (g_line_gap_bridge_active == 0U) {
        if ((g_line_last_error == 0) ||
    ...
    if (g_line_gap_bridge_count >= LINE_GAP_MAX_SAMPLES) {
        g_line_gap_bridge_active = 0U;
        return false;
    }
    ...
    g_turn_mode = 4U;
    g_line_correction = 0;
    set_pwm(LINE_GAP_LEFT_TICKS, LINE_GAP_RIGHT_TICKS);
    g_line_gap_bridge_count++;
    return true;
}
