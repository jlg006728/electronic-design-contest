/**
 * V46 encoder PID bridge, pulse line entry, stronger line turn.
 *
 * Scope:
 ...
 *   - Enter line-run when any black-line pulse appears in a 3-frame history.
 *   - Exit line-run when the current frame has no usable black-line bit.
 *   - All seven sensors low is accepted as black input; abnormal flag is watch-only.
...
#define START_GAP_LEFT_TRIM_TICKS   (-14)
#define START_GAP_RIGHT_TRIM_TICKS  14
...
#define START_GAP_ENCODER_LIMIT     115
...
static NOINLINE void read_line_sensors(void)
{
    ...
    g_line_entry_mask = g_line_detect_mask;
#if LINE_ENTRY_USE_PULSE_MASK
    g_line_entry_mask = (uint8_t) (g_line_entry_mask | black_pulse_mask);
#endif
...
static NOINLINE void update_line_position(void)
{
    ...
    uint8_t count = 0U;
    uint8_t mask =
        (g_line_detect_mask != 0U) ? g_line_detect_mask : g_line_entry_mask;
