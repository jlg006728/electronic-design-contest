/*
 * V89 stage 1: V87 baseline with startup bridge trim isolated.
 *
 * Wiring:
...
#define FIRMWARE_VERSION                 89U
#define V89_EXPERIMENT_STAGE             1U
...
#define LINE_ENTRY_CONFIRM_FRAMES        4U
...
#define CURVE_INNER_ENTRY_ERROR          (-1800)
#define CURVE_INNER_ENTRY_FRAMES         3U
#define CURVE_INNER_EXIT_ERROR           (-700)
#define CURVE_INNER_EXIT_FRAMES          2U
#define CURVE_TRACK_MIN_CORR_TICKS       8
#define CURVE_INNER_LEFT_LIMIT_TICKS     18
#define CURVE_ERROR_D_DIVISOR            30
#define CURVE_ERROR_D_LIMIT              35
...
#define CURVE_REENTRY_CONFIRM_FRAMES     4U
#define CURVE_NO_LINE_MAX_FRAMES         360U
#define GAP_REENTRY_MIN_FRAMES           50U
...
volatile uint8_t g_v89_experiment_stage = V89_EXPERIMENT_STAGE;
...
static NOINLINE void curve_update_control_state(int32_t error)
{
    ...
    if (g_curve_control_state == CURVE_CONTROL_INNER_RECOVERY) {
        if (error >= CURVE_INNER_EXIT_ERROR) {
    ...
    if (error <= CURVE_INNER_ENTRY_ERROR) {
...
static NOINLINE void line_follow_update(void)
{
    ...
    if ((g_curve_active != 0U) && !lost_recovery) {
        ...
        curve_update_control_state(error);
        g_curve_error_delta =
            error - g_curve_previous_control_error;
        if ((g_curve_control_state == CURVE_CONTROL_TRACK) &&
            (previous_control_state == CURVE_CONTROL_TRACK)) {
            curve_derivative = clamp_int32(
                g_curve_error_delta / CURVE_ERROR_D_DIVISOR,
                -CURVE_ERROR_D_LIMIT,
                CURVE_ERROR_D_LIMIT);
        }
...
int main(void)
{
    ...
    g_v89_experiment_stage = V89_EXPERIMENT_STAGE;
    safe_outputs_init();
    gray_sensor_init();
    pwm_init();
    motors_stop();
