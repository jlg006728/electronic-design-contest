/**
 * V47 encoder PID bridge, confirmed line entry, stronger line turn.
 *
 * Scope:
...
static NOINLINE void read_line_sensors(void)
{
    ...
    if ((g_line_detect_mask == 0U) &&
        ...
        pulse_candidate_mask = black_pulse_mask;
    }
#endif
    pulse_candidate_seen = (pulse_candidate_mask != 0U) ? 1U : 0U;
    ...
    if (g_line_detect_mask != 0U) {
        ...
        g_line_confirmed_entry_mask = g_line_detect_mask;
    } else if (g_line_candidate_confirm_count >=
        LINE_ENTRY_CANDIDATE_CONFIRM_MIN) {
        g_line_entry_mask = pulse_candidate_mask;
        g_line_confirmed_entry_mask = pulse_candidate_mask;
    } else {
        g_line_entry_mask = 0U;
    }
...
{
    uint8_t entry_mask =
        (g_line_entry_mask != 0U) ? g_line_entry_mask : g_line_detect_mask;
