...
#define START_BOOST_RIGHT_TICKS          568U
...
#define START_BRIDGE_GYRO_RATE_DIVISOR   1800
#define START_BRIDGE_GYRO_RATE_LIMIT     30
#define START_BRIDGE_HEADING_DEADBAND_MDEG 500
#define START_BRIDGE_HEADING_DIVISOR     500
#define START_BRIDGE_HEADING_LIMIT       35
#define START_BRIDGE_GYRO_TOTAL_LIMIT    40
#define START_BRIDGE_GYRO_SLEW_TICKS     4
...
static NOINLINE int32_t start_bridge_compute_mpu_correction(void)
{
    ...
    if (abs_int32(g_gyro_z_mdps) <=
        START_BRIDGE_GYRO_RATE_DEADBAND_MDPS) {
        g_start_bridge_gyro_rate_correction = 0;
    } else {
        g_start_bridge_gyro_rate_correction = clamp_int32(
            g_gyro_z_mdps / START_BRIDGE_GYRO_RATE_DIVISOR,
            -START_BRIDGE_GYRO_RATE_LIMIT,
            START_BRIDGE_GYRO_RATE_LIMIT);
    }

    if (abs_int32(g_start_bridge_heading_error_mdeg) <=
    ...
    desired_correction = clamp_int32(
        ...
        -START_BRIDGE_GYRO_TOTAL_LIMIT,
        START_BRIDGE_GYRO_TOTAL_LIMIT);
    correction_delta = clamp_int32(
        desired_correction - g_start_bridge_mpu_correction,
        -START_BRIDGE_GYRO_SLEW_TICKS,
        START_BRIDGE_GYRO_SLEW_TICKS);
    g_start_bridge_mpu_correction += correction_delta;
