/*
 * V65 MSPM0G3507 + degraded gray baseline + startup diagnostics.
 *
 * Wiring:
 ...
 *   The low-trigger buzzer reports boot and baseline status.
 *   OpenMV UART is not initialized.
 *
 * Startup:
 *   Place X1..X8 on the same white background before reset. The first
 *   50 scans establish an independent stable reference for each channel.
 */
...
#define GRAY_BASELINE_STABLE_LOW_MAX     5U
#define GRAY_BASELINE_STABLE_HIGH_MIN    45U
#define GRAY_BASELINE_MIN_STABLE_CHANNELS 7U
#define GRAY_MAIN_LOOP_MS                20U
#define GRAY_POWERUP_DELAY_MS            300U
#define DELAY_LOOPS_PER_US               8U

#define BOOT_SIGNAL_PULSES               3U
#define BOOT_SIGNAL_ON_MS                120U
#define BOOT_SIGNAL_OFF_MS               80U
#define STATUS_BEEP_ON_MS                90U
#define STATUS_BEEP_OFF_MS               80U
...
#define TURN_EXIT_BRAKE_CORR_TICKS       35
...
volatile uint8_t g_gray_baseline_stable_mask = 0U;
volatile uint8_t g_gray_baseline_unstable_mask = 0U;
volatile uint8_t g_gray_baseline_stable_count = 0U;
volatile uint8_t g_gray_baseline_usable = 0U;
volatile uint8_t g_gray_baseline_degraded = 0U;
...
static NOINLINE void startup_signal(void)
{
    for (uint8_t pulse = 0U; pulse < BOOT_SIGNAL_PULSES; pulse++) {
        led_set(true);
        buzzer_set(pulse == 0U);
        delay_ms_rough(BOOT_SIGNAL_ON_MS);
        led_set(false);
        buzzer_set(false);
        delay_ms_rough(BOOT_SIGNAL_OFF_MS);
    }
}

static NOINLINE void buzzer_beep_count(uint8_t count)
{
    for (uint8_t beep = 0U; beep < count; beep++) {
        buzzer_set(true);
        delay_ms_rough(STATUS_BEEP_ON_MS);
        buzzer_set(false);
        if ((beep + 1U) < count) {
            delay_ms_rough(STATUS_BEEP_OFF_MS);
        }
    }
}
...
static NOINLINE void gray_update_line_position(uint8_t changed_mask)
{
    ...
    g_gray_line_abnormal =
        ((count > LINE_MAX_ACTIVE) ||
         (changed_mask == g_gray_baseline_stable_mask)) ? 1U : 0U;
    if ((count == 0U) || (count > LINE_MAX_ACTIVE) ||
        (changed_mask == g_gray_baseline_stable_mask)) {
...
static NOINLINE void gray_update_baseline_and_changes(void)
{
    if (g_gray_baseline_ready == 0U) {
        ...
        if (g_gray_baseline_scan_count >= GRAY_BASELINE_SCAN_COUNT) {
            ...
            for (uint8_t channel = 0U; channel < GRAY_CHANNEL_COUNT; channel++) {
                uint16_t high_votes = g_gray_baseline_votes[channel];

                if (high_votes >=
                    ((GRAY_BASELINE_SCAN_COUNT + 1U) / 2U)) {
                    baseline_mask |= (uint8_t) (1U << channel);
                }
                if ((high_votes <= GRAY_BASELINE_STABLE_LOW_MAX) ||
                    (high_votes >= GRAY_BASELINE_STABLE_HIGH_MIN)) {
                    stable_mask |= (uint8_t) (1U << channel);
                    stable_count++;
                } else {
                    unstable_mask |= (uint8_t) (1U << channel);
                }
            ...
            g_gray_baseline_stable_mask = stable_mask;
            g_gray_baseline_unstable_mask = unstable_mask;
            g_gray_baseline_stable_count = stable_count;
            g_gray_baseline_usable =
                ((stable_count >= GRAY_BASELINE_MIN_STABLE_CHANNELS) &&
                 ((stable_mask & LINE_CENTER_MASK) == LINE_CENTER_MASK)) ? 1U : 0U;
            g_gray_baseline_degraded =
                ((g_gray_baseline_usable != 0U) &&
                 ((stable_count < GRAY_CHANNEL_COUNT) ||
                  (g_gray_baseline_uniform == 0U))) ? 1U : 0U;
            g_gray_white_level_code =
                (baseline_mask == 0x00U) ? 0U :
                (baseline_mask == 0xFFU) ? 1U : 2U;
            g_gray_baseline_ready = 1U;
            g_gray_test_state =
                (g_gray_baseline_usable == 0U) ? 3U :
                (g_gray_baseline_degraded != 0U) ? 4U : 2U;
    ...
    g_gray_changed_from_white_mask = (uint8_t)
        ((g_gray_raw_high_mask ^ g_gray_white_baseline_mask) &
         g_gray_baseline_stable_mask);
...
int main(void)
{
    ...
    buzzer_beep_count((g_gray_baseline_degraded != 0U) ? 3U : 2U);
