/*
 * V78 MSPM0G3507 + start-left and line-loss-right balance.
 *
 * Wiring:
...
#define START_BOOST_RIGHT_TICKS          569U
#define START_BOOST_FRAMES               20U
#define START_BRIDGE_LEFT_TICKS          422U
#define START_BRIDGE_RIGHT_TICKS         432U
...
#define TURN_EXIT_NUDGE_CORRECTION_TICKS 4
#define TURN_EXIT_NUDGE_FRAMES           4U
...
volatile uint8_t g_turn_exit_nudge_active = 0U;
volatile uint8_t g_turn_exit_nudge_remaining = 0U;
volatile uint32_t g_turn_exit_nudge_trigger_count = 0U;
...
static NOINLINE void curve_start(void)
{
    ...
    g_turn_exit_nudge_active = 0U;
    g_turn_exit_nudge_remaining = 0U;
    g_curve_start_count++;
}
...
static NOINLINE void line_follow_update(void)
{
    ...
    if (g_gray_changed_count != 0U) {
        ...
        ((mask_is_right_exit(g_line_mask_history_1) &&
          mask_is_right_exit(g_line_mask_history_2)) ||
         (g_line_last_nonzero_error > 0))) {
        g_turn_exit_nudge_active = 1U;
        g_turn_exit_nudge_remaining = TURN_EXIT_NUDGE_FRAMES;
        g_turn_exit_nudge_trigger_count++;
        g_line_last_nonzero_error = 0;
    }
    ...
    if (lost_recovery &&
        ...
        g_turn_mode = 4U;
        g_turn_was_active = 1U;
        g_line_correction = TURN_EXIT_NUDGE_CORRECTION_TICKS;
        g_gyro_rate_correction = 0;
        g_gyro_heading_correction = 0;
        g_mpu_control_correction = 0;
        ...
        set_pwm(
            ...
            clamp_pwm(
                (int32_t) PWM_RIGHT_BASE_TICKS -
                    g_total_control_correction));
        g_turn_exit_nudge_remaining--;
        if (g_turn_exit_nudge_remaining == 0U) {
            g_turn_exit_nudge_active = 0U;
        }
        return;
    }
    ...
    for (g_run_frame = 0U; g_run_frame < RUN_MAX_FRAMES; g_run_frame++) {
        ...
        if (g_phase == PHASE_START_BRIDGE) {
            ...
            if (g_start_bridge_frame_count <= START_BOOST_FRAMES) {
                ...
                set_pwm(
                    START_BRIDGE_LEFT_TICKS,
                    START_BRIDGE_RIGHT_TICKS);
            }
