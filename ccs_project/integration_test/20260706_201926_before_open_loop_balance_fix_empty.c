...
#define AIM_SEARCH_START_FRAMES     14U
#define AIM_SEARCH_STEP_US          18
...
#define PWM_LEFT_BASE_TICKS         408U
#define PWM_RIGHT_BASE_TICKS        398U
#define PWM_LEFT_BOOST_TICKS        408U
#define PWM_RIGHT_BOOST_TICKS       398U
#define PWM_MIN_TICKS               287U
#define PWM_MAX_TICKS               567U
#define PWM_SHARP_LEFT_BASE_TICKS   408U
#define PWM_SHARP_RIGHT_BASE_TICKS  398U
...
static int32_t boost_right_overshoot_correction(int32_t correction, int32_t limit)
{
    if (correction >= 0) {
        return correction;
    }
    ...
    return clamp_int32(
        (correction * LINE_RIGHT_OVERSHOOT_BOOST_NUM) /
            LINE_RIGHT_OVERSHOOT_BOOST_DEN,
        -limit,
        limit);
}
...
static NOINLINE void line_follow_update(void)
{
    ...
    if (sharp_turn) {
        g_line_correction = clamp_int32(
            ...
            LINE_SHARP_CORR_LIMIT);
        g_line_correction = boost_right_overshoot_correction(
            g_line_correction,
            LINE_SHARP_CORR_LIMIT);
