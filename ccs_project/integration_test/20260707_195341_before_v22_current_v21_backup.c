...
volatile uint8_t g_line_gap_bridge_active = 0;
volatile uint8_t g_line_gap_reacquire_count = 0;
volatile uint32_t g_line_gap_bridge_frames = 0;
volatile uint32_t g_line_gap_bridge_event_count = 0;
volatile int16_t g_line_gap_entry_error = 0;
volatile int32_t g_line_gap_left_start = 0;
volatile int32_t g_line_gap_right_start = 0;
volatile int32_t g_line_gap_left_forward = 0;
volatile int32_t g_line_gap_right_forward = 0;
volatile int32_t g_line_gap_balance_error = 0;
volatile int32_t g_line_gap_correction = 0;
volatile uint8_t g_line_gap_window_ok = 0;
volatile uint8_t g_line_gap_edge_ok = 0;
volatile uint8_t g_line_gap_encoder_seen = 0;
...
static bool line_gap_sample_in_window(uint32_t lap_sample, uint32_t center)
{
    uint32_t start = (center > LINE_GAP_WINDOW_MARGIN) ?
        (center - LINE_GAP_WINDOW_MARGIN) : 0U;
    uint32_t end = center + LINE_GAP_WINDOW_EXTRA;

    if (end >= COURSE_LAP_SAMPLES) {
        end = COURSE_LAP_SAMPLES - 1U;
    }

    return (lap_sample >= start) && (lap_sample <= end);
}

static bool line_gap_course_window_ok(void)
{
    uint32_t lap_sample = g_run_sample % COURSE_LAP_SAMPLES;
    bool ok =
        line_gap_sample_in_window(lap_sample, COURSE_POINT_B_SAMPLE) ||
        line_gap_sample_in_window(lap_sample, COURSE_POINT_D_SAMPLE);

    g_line_gap_window_ok = ok ? 1U : 0U;
    return ok;
}

static NOINLINE void line_gap_start_bridge(void)
{
    __disable_irq();
    g_line_gap_left_start = g_left_count;
    g_line_gap_right_start = g_right_count;
    __enable_irq();

    g_line_gap_bridge_active = 1U;
    g_line_gap_reacquire_count = 0U;
    g_line_gap_bridge_frames = 0U;
    g_line_gap_entry_error = g_line_last_error;
    g_line_gap_bridge_event_count++;
}

static NOINLINE void line_gap_update_encoder_balance(void)
{
    int32_t left_now;
    int32_t right_now;
    ...
    g_line_gap_left_forward = -(left_now - g_line_gap_left_start);
    g_line_gap_right_forward = right_now - g_line_gap_right_start;
    g_line_gap_balance_error =
        g_line_gap_left_forward - g_line_gap_right_forward;
    g_line_gap_correction = clamp_int32(
        g_line_gap_balance_error / LINE_GAP_KP_DIVISOR,
        -LINE_GAP_CORR_LIMIT,
        LINE_GAP_CORR_LIMIT);
...
static NOINLINE bool line_gap_bridge_handle(void)
{
    if (LINE_GAP_BRIDGE_ENABLE == 0U) {
        return false;
    }

    if (g_line_gap_bridge_active == 0U) {
        g_line_gap_edge_ok =
            (abs_int32((int32_t) g_line_last_error) >= LINE_GAP_ENTER_ERROR) ?
            1U : 0U;

        if ((g_line_valid != 0U) ||
            (g_line_last_error == 0) ||
            (g_line_gap_edge_ok == 0U) ||
            !line_gap_course_window_ok()) {
            return false;
        }

        line_gap_start_bridge();
    } else if (g_line_valid != 0U) {
        if (g_line_gap_reacquire_count < 0xFFU) {
            g_line_gap_reacquire_count++;
        }

        if (g_line_gap_reacquire_count >= LINE_GAP_REACQUIRE_FRAMES) {
            g_line_gap_bridge_active = 0U;
            g_line_gap_bridge_frames = 0U;
            g_line_gap_reacquire_count = 0U;
            return false;
        }
    ...
    if (g_line_gap_bridge_frames >= LINE_GAP_MAX_FRAMES) {
        g_line_gap_bridge_active = 0U;
        return false;
    }

    line_gap_update_encoder_balance();
    g_turn_mode = 4U;
    g_line_correction = g_line_gap_correction;
    set_pwm(
        clamp_pwm((int32_t) LINE_GAP_LEFT_BASE_TICKS - g_line_gap_correction),
        clamp_pwm((int32_t) LINE_GAP_RIGHT_BASE_TICKS + g_line_gap_correction));
