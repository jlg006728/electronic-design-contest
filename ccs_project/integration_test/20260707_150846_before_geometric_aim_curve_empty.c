...
volatile uint8_t g_course_segment = COURSE_SEGMENT_AB;
volatile uint16_t g_segment_aim_x_us = SERVO_X_HOME_US;
...
static uint8_t course_segment_from_sample(void)
{
    if (g_run_sample < MISSION_B_STOP_SAMPLE) {
        return COURSE_SEGMENT_AB;
    }
    if (g_run_sample < MISSION_C_PROMPT_SAMPLE) {
        return COURSE_SEGMENT_BC;
    }
    if (g_run_sample < MISSION_D_PROMPT_SAMPLE) {
        return COURSE_SEGMENT_CD;
    }
    return COURSE_SEGMENT_DA;
}

static uint16_t course_segment_target_x_us(void)
{
    uint8_t segment = course_segment_from_sample();
    uint16_t target = g_servo_x_home_us;

    if (segment == COURSE_SEGMENT_BC) {
        target = COURSE_SEGMENT_BC_LEFT_US;
    } else if (segment == COURSE_SEGMENT_CD) {
        target = COURSE_SEGMENT_CD_RETURN_US;
    } else if (segment == COURSE_SEGMENT_DA) {
        target = COURSE_SEGMENT_DA_RIGHT_US;
    }

    g_course_segment = segment;
    g_segment_aim_x_us = clamp_servo(target, SERVO_X_MIN_US, SERVO_X_MAX_US);
    return g_segment_aim_x_us;
}
...
static NOINLINE void aim_search_for_lost_target(void)
{
    ...
    if (COURSE_SEGMENT_AIM_ENABLE != 0U) {
        uint16_t target_x = course_segment_target_x_us();

        if (g_servo_x_us + COURSE_SEGMENT_AIM_STEP_US < target_x) {
            next_x = (int32_t) g_servo_x_us + COURSE_SEGMENT_AIM_STEP_US;
        } else if (g_servo_x_us > target_x + COURSE_SEGMENT_AIM_STEP_US) {
            next_x = (int32_t) g_servo_x_us - COURSE_SEGMENT_AIM_STEP_US;
        } else {
            next_x = target_x;
        }
...
int main(void)
{
    ...
    while (1) {
        ...
        servo_output_one_frame();
        if (COURSE_SEGMENT_AIM_ENABLE != 0U) {
            (void) course_segment_target_x_us();
        }
