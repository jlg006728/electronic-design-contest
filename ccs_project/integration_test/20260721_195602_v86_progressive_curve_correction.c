/*
 * V86 MSPM0G3507 + progressive right-curve correction.
 *
 * Wiring:
...
#define START_BOOST_LEFT_TICKS           560U
...
#define CURVE_RIGHT_LEFT_CORR_LIMIT      60
#define CURVE_ERROR_FILTER_DIVISOR       3
#define CURVE_RIGHT_BUILD_TICKS          36
#define CURVE_RIGHT_RELEASE_TICKS        24
#define CURVE_LEFT_BUILD_TICKS           14
#define CURVE_LEFT_RELEASE_TICKS         18
...
volatile int32_t g_curve_filtered_error = 0;
...
static NOINLINE void curve_start(void)
{
    ...
    g_curve_filtered_error = g_gray_line_error;
    ...
    g_curve_last_valid_total_correction = g_total_control_correction;
    ...
    int32_t desired_correction)
{
    ...
    if (desired_correction > current_correction) {
        step_limit = (current_correction < 0)
            ? CURVE_LEFT_RELEASE_TICKS
            : CURVE_RIGHT_BUILD_TICKS;
        correction_delta = clamp_int32(
            desired_correction - current_correction,
            0,
            step_limit);
        ...
        step_limit = (current_correction > 0)
            ? CURVE_RIGHT_RELEASE_TICKS
            : CURVE_LEFT_BUILD_TICKS;
        correction_delta = clamp_int32(
            desired_correction - current_correction,
            -step_limit,
            0);
...
static NOINLINE void line_follow_update(void)
{
    ...
    if ((g_curve_active != 0U) && !lost_recovery) {
        g_curve_filtered_error =
            (((CURVE_ERROR_FILTER_DIVISOR - 1) *
                g_curve_filtered_error) +
             (int32_t) g_gray_line_error) /
            CURVE_ERROR_FILTER_DIVISOR;
        error = g_curve_filtered_error;
    }
    sharp_turn =
    ...
    if (sharp_turn) {
        ...
        g_total_control_correction =
            g_line_correction + curve_feedforward +
                curve_right_recovery + gyro_correction;
        if ((g_curve_active != 0U) && !lost_recovery) {
            g_total_control_correction =
    ...
    g_line_correction = clamp_int32(
        ...
        LINE_CORR_LIMIT);
    if ((g_curve_active != 0U) &&
        (g_line_correction < -CURVE_RIGHT_LEFT_CORR_LIMIT)) {
        g_line_correction = -CURVE_RIGHT_LEFT_CORR_LIMIT;
    }
