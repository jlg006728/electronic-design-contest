...
#define ENCODER_DISTANCE_ENABLE     1U
...
#define COURSE_POINT_B_EDGES        (COURSE_POINT_B_CM * ENCODER_EDGES_PER_CM)
#define COURSE_POINT_C_EDGES        (COURSE_POINT_C_CM * ENCODER_EDGES_PER_CM)
#define COURSE_POINT_D_EDGES        (COURSE_POINT_D_CM * ENCODER_EDGES_PER_CM)
#define COURSE_POINT_A_EDGES        (COURSE_LAP_CM * ENCODER_EDGES_PER_CM)
#define COURSE_LAP_EDGES            COURSE_POINT_A_EDGES
...
volatile uint32_t g_encoder_distance_edges = 0;
volatile uint32_t g_encoder_lap_edges = 0;
volatile uint32_t g_point_stop_lap_base_edges = 0;
volatile uint32_t g_point_stop_target_edges = 0;
...
static NOINLINE void encoder_reset_counts(void)
{
    ...
    g_left_edges = 0;
    g_right_edges = 0;
    g_encoder_distance_edges = 0U;
    g_encoder_lap_edges = 0U;
    g_point_stop_lap_base_edges = 0U;
    g_point_stop_target_edges = 0U;
    DL_GPIO_clearInterruptStatus(ENCODER_PORT, ENCODER_INT_MASK);
    __enable_irq();
}

static uint32_t encoder_forward_distance_edges(void)
{
    int32_t left_forward;
    int32_t right_forward;
    uint32_t left_edges_forward;
    uint32_t right_edges_forward;

    __disable_irq();
    left_forward = -g_left_count;
    right_forward = g_right_count;
    __enable_irq();

    left_edges_forward = (left_forward > 0) ? (uint32_t) left_forward : 0U;
    right_edges_forward = (right_forward > 0) ? (uint32_t) right_forward : 0U;

    g_encoder_distance_edges =
        (left_edges_forward + right_edges_forward) / 2U;
    if (COURSE_LAP_EDGES != 0U) {
        g_encoder_lap_edges = g_encoder_distance_edges % COURSE_LAP_EDGES;
    } else {
        g_encoder_lap_edges = 0U;
    }

    return g_encoder_distance_edges;
}
...
static uint32_t course_current_lap_sample(void)
{
    if (ENCODER_DISTANCE_ENABLE != 0U) {
        uint32_t lap_edges;

        (void) encoder_forward_distance_edges();
        lap_edges = g_encoder_lap_edges;
        if (COURSE_LAP_EDGES == 0U) {
            return 0U;
        }
        return (lap_edges * COURSE_LAP_SAMPLES) / COURSE_LAP_EDGES;
    }

    return g_run_sample % COURSE_LAP_SAMPLES;
}

static uint8_t course_preaim_phase_from_sample(void)
{
    uint32_t lap_sample = course_current_lap_sample();

    g_course_lap_sample = lap_sample;
    if (lap_sample < COURSE_BC_MID_SAMPLE) {
...
static uint16_t course_preaim_target_from_lap_sample(uint32_t lap_sample)
{
    ...
    if (lap_sample >= COURSE_BC_MID_SAMPLE) {
        uint32_t return_span = COURSE_LAP_SAMPLES - COURSE_BC_MID_SAMPLE;
        uint32_t return_sample = lap_sample - COURSE_BC_MID_SAMPLE;
        int32_t remaining = 0;
...
static uint32_t course_point_relative_edges(uint8_t point_index)
{
    switch (point_index) {
    case 0U:
        return COURSE_POINT_B_EDGES;
    case 1U:
        return COURSE_POINT_C_EDGES;
    case 2U:
        return COURSE_POINT_D_EDGES;
    default:
        return COURSE_POINT_A_EDGES;
    }
}
...
static NOINLINE void mission_enter_point_hold(void)
{
    ...
    g_point_stop_target_edges =
        g_point_stop_lap_base_edges + course_point_relative_edges(point_index);
...
static bool mission_try_enter_point_hold(void)
{
    ...
    if (ENCODER_DISTANCE_ENABLE != 0U) {
        target_edges = g_point_stop_lap_base_edges +
            course_point_relative_edges(g_point_stop_next_index);
        g_point_stop_target_edges = target_edges;
        if (encoder_forward_distance_edges() < target_edges) {
            return false;
        }
...
static NOINLINE void mission_resume_after_point_hold(void)
{
    ...
    if (g_point_hold_index >= 3U) {
        ...
        g_point_stop_lap_base_edges += COURSE_LAP_EDGES;
    } else {
