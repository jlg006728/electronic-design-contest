/**
 * V40 history-triggered line test with encoder-balanced bridge and OpenMV servo.
 *
 * Scope:
 *   - Chassis/line-following test only, no ABCD mission logic.
 *   - White surface = sensor high/1/light on; black line = low/0/light off.
 *   - Bridge stage uses encoder balance to reduce open-loop drift.
 *   - Enter line-run when any usable black-line bit appears in a 3-frame history.
 *   - Exit line-run when the current frame has no usable black-line bit.
 *   - All seven sensors low is treated as abnormal/air/all-black, not as centered line.
...
#define START_GAP_LEFT_BASE_TICKS   245U
#define START_GAP_RIGHT_BASE_TICKS  250U
#define START_GAP_MAX_FRAMES        900U
#define START_GAP_ENCODER_KP_DIV    16
#define START_GAP_ENCODER_LIMIT     80
...
#define LINE_ENTER_MAX_ACTIVE       7U
#define SENSOR_ALL_LOW_STOP_FRAMES  0U
...
static NOINLINE void update_line_history(void)
{
    ...
    g_line_history0 = g_line_detect_mask;
...
static NOINLINE void update_line_position(void)
{
    int32_t sum = 0;
    uint8_t count = 0U;
    uint8_t mask = g_line_detect_mask;
...
static NOINLINE void start_gap_bridge_update(void)
{
    ...
    correction = clamp_int32(
        g_encoder_diff_forward_total / START_GAP_ENCODER_KP_DIV,
        -START_GAP_ENCODER_LIMIT,
        START_GAP_ENCODER_LIMIT);
    g_start_gap_encoder_correction = correction;
    g_turn_mode = 4U;
    g_line_correction = correction;
    set_pwm(
        clamp_pwm((int32_t) START_GAP_LEFT_BASE_TICKS - correction),
        clamp_pwm((int32_t) START_GAP_RIGHT_BASE_TICKS + correction));
}
...
static NOINLINE void hardware_init(void)
{
    ...
    init_output(IOMUX_PINCM17, MOTOR_PORT, STBY_PIN,
        (MOTOR_ENABLE != 0U) ? true : false);
