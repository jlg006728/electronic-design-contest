/*
 * MSPM0G3507 + line following + OpenMV dual-servo laser aiming integration.
 *
 * V23: ring-center OpenMV target + laser aiming tuning.
 *
 * Goal:
 *   Keep the verified line-following chassis running while the PA14/PA17
 *   gimbal tracks target-center coordinates streamed by OpenMV.
 *
 * OpenMV script:
 *   ccs_project/openmv_test/
 *   20260708_openmv_uart_stream_ring_center_v1.py
 *
 * Protocol note:
 *   UART frame format is unchanged. OpenMV still sends cx/cy/pixels, but cx/cy
 *   now represent the concentric-ring target center instead of a red blob
 *   centroid.
 *
 * Wiring:
...
#define AIM_CENTER_X                (IMG_CENTER_X + LASER_CALIB_X_PIXELS)
#define AIM_CENTER_Y                (IMG_CENTER_Y + LASER_CALIB_Y_PIXELS)
...
volatile int16_t g_aim_filtered_cx = AIM_CENTER_X;
volatile int16_t g_aim_filtered_cy = AIM_CENTER_Y;
...
static NOINLINE void servo_go_home_now(void)
{
    ...
    g_aim_dx = 0;
    g_aim_dy = 0;
    g_aim_filtered_cx = AIM_CENTER_X;
    g_aim_filtered_cy = AIM_CENTER_Y;
    g_aim_x_step_us = 0;
    g_aim_y_step_us = 0;
    g_aim_filter_ready = 0U;
    g_aim_target_confirm_count = 0U;
...
static NOINLINE void aiming_update_from_latest_color_frame(void)
{
    ...
    g_aim_raw_dx = (int16_t) (g_omv_cx - AIM_CENTER_X);
    g_aim_raw_dy = (int16_t) (g_omv_cy - AIM_CENTER_Y);
    ...
    dx = (int16_t) (g_aim_filtered_cx - AIM_CENTER_X);
    dy = (int16_t) (g_aim_filtered_cy - AIM_CENTER_Y);
    g_aim_dx = dx;
    g_aim_dy = dy;
