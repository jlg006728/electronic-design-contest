...
static uint16_t course_preaim_target_x_us(void)
{
    ...
    if (phase == COURSE_PREAIM_RETURN_HOME) {
        uint32_t return_span = COURSE_LAP_SAMPLES - COURSE_BC_MID_SAMPLE;
        uint32_t return_sample = g_course_lap_sample - COURSE_BC_MID_SAMPLE;
        int32_t remaining = 0;
    ...
    g_course_preaim_x_us = clamp_course_preaim_x(target);
...
static NOINLINE void aim_search_for_lost_target(void)
{
    ...
    if (COURSE_PREAIM_ENABLE != 0U) {
        uint16_t target_x = course_preaim_target_x_us();
