...
#define STATE_SWITCH_REASON_NONE    0U
#define STATE_SWITCH_REASON_BRIDGE  1U
#define STATE_SWITCH_REASON_REACQ   2U
#define STATE_SWITCH_REASON_LINE_AT_START 3U
...
#define START_GAP_SIMPLE_REACQUIRE  1U
...
#define START_GAP_LEFT_BASE_TICKS   PWM_LEFT_BASE_TICKS
#define START_GAP_RIGHT_BASE_TICKS  PWM_RIGHT_BASE_TICKS
...
#define PHASE_START_GAP_CAPTURE     9U
...
volatile uint8_t g_state_switch_reason = STATE_SWITCH_REASON_NONE;
...
volatile uint8_t g_start_gap_bridge_active = 0;
volatile uint8_t g_start_gap_reacquire_count = 0;
volatile uint8_t g_start_gap_encoder_seen = 0;
volatile uint8_t g_start_gap_center_lock_count = 0;
volatile uint8_t g_start_gap_capture_lost_count = 0;
volatile uint32_t g_start_gap_bridge_frames = 0;
volatile uint32_t g_start_gap_bridge_event_count = 0;
volatile uint32_t g_start_gap_capture_frames = 0;
volatile uint32_t g_start_gap_capture_event_count = 0;
volatile uint32_t g_start_gap_reacquired_event_count = 0;
volatile int32_t g_start_gap_left_start = 0;
volatile int32_t g_start_gap_right_start = 0;
volatile int32_t g_start_gap_left_forward = 0;
volatile int32_t g_start_gap_right_forward = 0;
volatile int32_t g_start_gap_balance_error = 0;
volatile int32_t g_start_gap_correction = 0;
...
static NOINLINE void state_switch_hold_then_resume(uint8_t reason)
{
    uint32_t left_pwm = g_left_pwm_ticks;
    uint32_t right_pwm = g_right_pwm_ticks;

    g_state_switch_reason = reason;
    g_state_switch_hold_event_count++;

    motors_stop();
    for (uint32_t i = 0U; i < STATE_SWITCH_HOLD_FRAMES; i++) {
...
static NOINLINE void start_gap_start_bridge(void)
{
    __disable_irq();
    g_start_gap_left_start = g_left_count;
    g_start_gap_right_start = g_right_count;
    __enable_irq();

    g_start_gap_bridge_active = 1U;
    g_start_gap_reacquire_count = 0U;
    g_start_gap_bridge_frames = 0U;
    g_start_gap_left_forward = 0;
    g_start_gap_right_forward = 0;
    g_start_gap_balance_error = 0;
    g_start_gap_correction = 0;
    g_start_gap_bridge_event_count++;
    buzzer_start_beeps(2U);
}

static uint32_t start_gap_left_drive_ticks(void)
{
    return clamp_pwm(
        (int32_t) START_GAP_LEFT_BASE_TICKS + START_GAP_LEFT_TRIM_TICKS);
}

static uint32_t start_gap_right_drive_ticks(void)
{
    return clamp_pwm(
        (int32_t) START_GAP_RIGHT_BASE_TICKS + START_GAP_RIGHT_TRIM_TICKS);
}

static NOINLINE void start_gap_update_encoder_balance(void)
{
    int32_t left_now;
    int32_t right_now;

    __disable_irq();
    left_now = g_left_count;
    right_now = g_right_count;
    __enable_irq();

    g_start_gap_left_forward = -(left_now - g_start_gap_left_start);
    g_start_gap_right_forward = right_now - g_start_gap_right_start;
    g_start_gap_balance_error =
        g_start_gap_left_forward - g_start_gap_right_forward;
    g_start_gap_correction = clamp_int32(
        g_start_gap_balance_error / START_GAP_KP_DIVISOR,
        -START_GAP_CORR_LIMIT,
        START_GAP_CORR_LIMIT);
...
{
    if (START_GAP_BRIDGE_ENABLE == 0U) {
        g_phase = PHASE_LINE_RUN;
        return;
    }

    if (g_start_gap_bridge_active == 0U) {
        start_gap_start_bridge();
    }

    if (g_line_valid != 0U) {
        if (g_start_gap_reacquire_count < 0xFFU) {
            g_start_gap_reacquire_count++;
        }

        if (g_start_gap_reacquire_count >= START_GAP_REACQUIRE_FRAMES) {
            g_start_gap_bridge_active = 0U;
            g_start_gap_reacquire_count = 0U;
            g_start_gap_center_lock_count = 0U;
            g_start_gap_capture_lost_count = 0U;
            g_start_gap_capture_frames = 0U;
            g_start_gap_reacquired_event_count++;
            g_line_lost_count = 0U;
            line_follow_update();
            g_phase = PHASE_LINE_RUN;
            buzzer_start_beeps(3U);
            state_switch_hold_then_resume(STATE_SWITCH_REASON_REACQ);
            return;
        }
    } else {
        g_start_gap_reacquire_count = 0U;
    ...
    if (g_start_gap_bridge_frames >= START_GAP_MAX_FRAMES) {
        g_start_gap_bridge_active = 0U;
        g_phase = 4U;
        g_stop_reason = 6U;
        motors_stop();
    ...
    start_gap_update_encoder_balance();
    g_turn_mode = 4U;
    if (START_GAP_USE_ENCODER_BALANCE == 0U) {
        g_start_gap_correction = 0;
    }
    g_line_correction = g_start_gap_correction;
    set_pwm(
        clamp_pwm((int32_t) start_gap_left_drive_ticks() - g_start_gap_correction),
        clamp_pwm((int32_t) start_gap_right_drive_ticks() + g_start_gap_correction));
    g_start_gap_bridge_frames++;
}

static bool start_gap_center_locked(void)
{
    bool center_sensor_seen =
        (g_s3 != 0U) || (g_s4 != 0U) || (g_s5 != 0U);

    return (g_line_valid != 0U) &&
        center_sensor_seen &&
        (abs_int32((int32_t) g_line_error) <= START_GAP_CENTER_ERROR_LIMIT);
}

static NOINLINE void start_gap_capture_drive(void)
{
    int32_t error = (int32_t) g_line_error;

    g_line_correction = clamp_int32(
        error / START_GAP_CAPTURE_KP_DIVISOR,
        -START_GAP_CAPTURE_CORR_LIMIT,
        START_GAP_CAPTURE_CORR_LIMIT);

    set_pwm(
        clamp_pwm((int32_t) START_GAP_CAPTURE_LEFT_BASE_TICKS - g_line_correction),
        clamp_pwm((int32_t) START_GAP_CAPTURE_RIGHT_BASE_TICKS + g_line_correction));
}

static NOINLINE void start_gap_capture_update(void)
{
    read_line_sensors();
    update_line_position();
    g_turn_mode = 5U;

    if (start_gap_center_locked()) {
        if (g_start_gap_center_lock_count < 0xFFU) {
    ...
    if (g_start_gap_center_lock_count >= START_GAP_CENTER_LOCK_FRAMES) {
        g_start_gap_reacquired_event_count++;
        g_start_gap_capture_lost_count = 0U;
        g_start_gap_capture_frames = 0U;
        g_start_gap_center_lock_count = 0U;
    ...
    if (g_line_valid != 0U) {
        g_start_gap_capture_lost_count = 0U;
    } else if (g_start_gap_capture_lost_count < 0xFFU) {
        g_start_gap_capture_lost_count++;
    }

    if ((g_start_gap_capture_lost_count > START_GAP_CAPTURE_LOST_MAX_FRAMES) ||
        (g_start_gap_capture_frames >= START_GAP_CAPTURE_MAX_FRAMES)) {
        g_start_gap_reacquire_count = 0U;
        g_start_gap_center_lock_count = 0U;
        g_start_gap_capture_lost_count = 0U;
        g_start_gap_bridge_active = 1U;
...
static uint16_t clamp_course_preaim_x(int32_t value)
{
    ...
    int32_t right = (int32_t) COURSE_RIGHT_NEAR_TARGET_US;
...
static uint8_t course_preaim_phase_from_sample(void)
{
    ...
    if (lap_sample <= COURSE_POINT_B_SAMPLE) {
        return COURSE_PREAIM_LEFT_HOLD;
    }
    if (lap_sample < COURSE_LEFT_HOLD_SAMPLE) {
        return COURSE_PREAIM_FAST_RIGHT;
    }
    if (lap_sample < COURSE_POINT_D_SAMPLE) {
        return COURSE_PREAIM_RIGHT_DEEPEN;
    }
    return COURSE_PREAIM_RETURN_HOME;
}

static int32_t interpolate_course_x(
    uint32_t sample,
    ...
    int32_t end_x)
{
    ...
    progress = sample - start_sample;
    delta = end_x - start_x;
    return start_x + ((delta * (int32_t) progress) / (int32_t) span);
}
...
static uint16_t course_preaim_target_from_lap_sample(uint32_t lap_sample)
{
    int32_t home = (int32_t) COURSE_HOME_TARGET_US;
    int32_t left = (int32_t) COURSE_LEFT_TARGET_US;
    int32_t right_near = (int32_t) COURSE_RIGHT_NEAR_TARGET_US;
    int32_t right_mid = (int32_t) COURSE_RIGHT_MID_TARGET_US;
    int32_t target = left;

    if (lap_sample <= COURSE_POINT_B_SAMPLE) {
        target = left;
    } else if (lap_sample < COURSE_LEFT_HOLD_SAMPLE) {
        target = interpolate_course_x(
            lap_sample,
            COURSE_POINT_B_SAMPLE,
            COURSE_LEFT_HOLD_SAMPLE,
            right_near,
            right_mid);
    } else if (lap_sample < COURSE_POINT_D_SAMPLE) {
        target = interpolate_course_x(
            lap_sample,
            COURSE_LEFT_HOLD_SAMPLE,
...
static NOINLINE void aim_search_for_lost_target(void)
{
    ...
    if (COURSE_PREAIM_ENABLE != 0U) {
        ...
        } else if (g_servo_x_us > target_x + COURSE_PREAIM_RETURN_STEP_US) {
            next_x = (int32_t) g_servo_x_us - COURSE_PREAIM_RETURN_STEP_US;
        } else {
            next_x = target_x;
        }
...
int main(void)
{
    ...
    if ((START_GAP_BRIDGE_ENABLE != 0U) && (g_line_valid == 0U)) {
        g_turn_mode = 4U;
        g_line_correction = 0;
        start_gap_start_bridge();
        set_pwm(start_gap_left_drive_ticks(), start_gap_right_drive_ticks());
        g_phase = PHASE_START_GAP_BRIDGE;
        state_switch_hold_then_resume(STATE_SWITCH_REASON_BRIDGE);
    } else {
        line_follow_update();
        g_phase = PHASE_LINE_RUN;
        buzzer_start_beeps(4U);
        state_switch_hold_then_resume(STATE_SWITCH_REASON_LINE_AT_START);
    }
    ...
    while (1) {
        ...
        if (g_phase == PHASE_START_GAP_CAPTURE) {
            start_gap_capture_update();
            continue;
        }
        ...
        if (g_phase == PHASE_LINE_RUN) {
            if ((START_GAP_TEST_ONLY == 0U) &&
                mission_try_enter_point_hold()) {
                continue;
            }

            if ((START_GAP_TEST_ONLY == 0U) &&
                (MISSION_ENABLE_B_AIM != 0U) &&
                (g_mission_b_done == 0U) &&
            ...
            if ((START_GAP_TEST_ONLY == 0U) &&
                (MISSION_ENABLE_POINT_PROMPTS != 0U)) {
