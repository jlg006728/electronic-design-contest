/*
 * V84 MSPM0G3507 + mature curve-exit discrimination.
 *
 * Wiring:
...
#define TURN_EXIT_CONFIRM_LOST_FRAMES    10U
...
#define CURVE_RIGHT_LOST_HOLD_ERROR      1800
#define CURVE_RIGHT_LEFT_CORR_LIMIT      60
#define CURVE_MATURE_EXIT_MIN_FRAMES     280U
#define CURVE_MATURE_EXIT_CONFIRM_FRAMES 4U
#define CURVE_MATURE_EXIT_ERROR_MAX      1800
...
volatile uint32_t g_curve_active_frame_count = 0U;
volatile uint32_t g_curve_exit_frame_snapshot = 0U;
volatile int16_t g_curve_previous_valid_error = 0;
volatile int16_t g_curve_last_valid_error = 0;
volatile uint8_t g_curve_mature_exit_history_aligned = 0U;
volatile uint8_t g_curve_mature_exit_active = 0U;
volatile uint8_t g_curve_mature_exit_confirm_count = 0U;
volatile uint32_t g_curve_mature_exit_trigger_count = 0U;
volatile uint32_t g_curve_mature_exit_confirmed_count = 0U;
volatile uint32_t g_curve_mature_exit_cancel_count = 0U;
...
static NOINLINE void curve_start(void)
{
    ...
    g_curve_active_frame_count = 0U;
    g_curve_previous_valid_error = g_gray_line_error;
    g_curve_last_valid_error = g_gray_line_error;
    g_curve_mature_exit_history_aligned = 0U;
    g_curve_mature_exit_active = 0U;
    g_curve_mature_exit_confirm_count = 0U;
...
static NOINLINE void curve_update_measurement(void)
{
    if ((g_curve_active != 0U) &&
        (g_curve_active_frame_count < UINT32_MAX)) {
        g_curve_active_frame_count++;
    }
...
static NOINLINE void curve_finish(void)
{
    ...
    g_curve_exit_frame_snapshot = g_curve_active_frame_count;
    g_curve_active = 0U;
    g_curve_feedforward_correction = 0;
    g_curve_right_recovery_correction = 0;
    g_curve_active_frame_count = 0U;
    g_curve_previous_valid_error = 0;
    g_curve_last_valid_error = 0;
    g_curve_mature_exit_history_aligned = 0U;
    g_curve_mature_exit_active = 0U;
    g_curve_mature_exit_confirm_count = 0U;
    g_curve_lost_hold_active = 0U;
...
static NOINLINE bool curve_mature_exit_handle(bool lost_recovery)
{
    bool history_aligned;
    ...
    if (!lost_recovery) {
        if (g_curve_mature_exit_active != 0U) {
            g_curve_mature_exit_cancel_count++;
        }
        g_curve_mature_exit_active = 0U;
        g_curve_mature_exit_confirm_count = 0U;
        g_curve_mature_exit_history_aligned = 0U;
        g_curve_previous_valid_error = g_curve_last_valid_error;
        g_curve_last_valid_error = g_gray_line_error;
        return false;
    }
    ...
    if (g_curve_mature_exit_active == 0U) {
        ...
        history_aligned =
            (abs_int32(g_curve_previous_valid_error) <=
                CURVE_MATURE_EXIT_ERROR_MAX) &&
            (abs_int32(g_curve_last_valid_error) <=
                CURVE_MATURE_EXIT_ERROR_MAX);
        g_curve_mature_exit_history_aligned = history_aligned ? 1U : 0U;
        if ((g_curve_active_frame_count <
             CURVE_MATURE_EXIT_MIN_FRAMES) || !history_aligned) {
            return false;
        }

        g_curve_mature_exit_active = 1U;
        g_curve_mature_exit_confirm_count = 1U;
        g_curve_mature_exit_trigger_count++;
    } else if (g_curve_mature_exit_confirm_count <
        CURVE_MATURE_EXIT_CONFIRM_FRAMES) {
        g_curve_mature_exit_confirm_count++;
    }

    /* A mature curve ending is a tangent transition, not a lost-line turn. */
    g_curve_lost_hold_active = 0U;
    g_turn_exit_candidate_direction = 0;
    g_turn_exit_candidate_lost_count = 0U;
    g_turn_exit_brake_active = 0U;
    ...
    if (g_curve_mature_exit_confirm_count >=
        CURVE_MATURE_EXIT_CONFIRM_FRAMES) {
        g_curve_mature_exit_confirmed_count++;
        g_line_last_nonzero_error = 0;
        curve_finish();
        set_pwm(PWM_LEFT_BASE_TICKS, PWM_RIGHT_BASE_TICKS);
    }
...
static NOINLINE void line_follow_update(void)
{
    ...
    if (curve_mature_exit_handle(lost_recovery)) {
        return;
    }
    ...
    if (lost_recovery &&
        ...
        if (error < CURVE_RIGHT_LOST_HOLD_ERROR) {
            error = CURVE_RIGHT_LOST_HOLD_ERROR;
        }
        sharp_turn = true;
        g_line_last_nonzero_error = (int16_t) error;
    }
