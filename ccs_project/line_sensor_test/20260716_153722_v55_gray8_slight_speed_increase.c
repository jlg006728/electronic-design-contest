/*
 * V55 MSPM0G3507 + YB-MYX05-V1.0 slight-speed-increase line-follow test.
 *
 * Wiring:
 *   PB5 -> AD0, PB6 -> AD1, PB7 -> AD2, PA7 <- OUT.
 *
 * Scope:
 *   The grayscale array, TB6612 motors and wheel encoders are active.
 *   OpenMV, servos and mission logic remain disabled.
...
#define PWM_LEFT_BASE_TICKS              340U
#define PWM_RIGHT_BASE_TICKS             340U
#define PWM_MIN_TICKS                    160U
#define PWM_MAX_TICKS                    560U
#define START_BOOST_LEFT_TICKS           480U
#define START_BOOST_RIGHT_TICKS          480U
#define START_BOOST_FRAMES               8U
#define LINE_KP_DIVISOR                  18
#define LINE_CORR_LIMIT                  100
#define LINE_SHARP_ERROR                 1800
#define LINE_SHARP_KP_DIVISOR            8
#define LINE_SHARP_CORR_LIMIT            210
...
#define BRIDGE_ENCODER_KP_DIVISOR        48
#define BRIDGE_ENCODER_KI_DIVISOR        1800
#define BRIDGE_ENCODER_KD_DIVISOR        2
#define BRIDGE_ENCODER_INTEGRAL_LIMIT    12000
#define BRIDGE_ENCODER_CORR_LIMIT        110
#define BRIDGE_ENCODER_SLEW_LIMIT        10
...
volatile int32_t g_bridge_encoder_error = 0;
volatile int32_t g_bridge_left_delta = 0;
volatile int32_t g_bridge_right_delta = 0;
volatile int32_t g_bridge_delta_error = 0;
volatile int32_t g_bridge_integral_error = 0;
volatile int32_t g_bridge_p_term = 0;
volatile int32_t g_bridge_i_term = 0;
volatile int32_t g_bridge_d_term = 0;
volatile int32_t g_bridge_target_correction = 0;
volatile int32_t g_bridge_encoder_correction = 0;
volatile uint32_t g_bridge_left_command = PWM_LEFT_BASE_TICKS;
volatile uint32_t g_bridge_right_command = PWM_RIGHT_BASE_TICKS;
volatile uint8_t g_bridge_encoder_seen = 0U;
...
static NOINLINE void encoder_update_forward_totals(void)
{
    ...
    /* Confirmed vehicle convention: left forward is negative, right positive. */
    g_left_forward_total = -left_count;
    g_right_forward_total = right_count;
    g_bridge_encoder_seen =
        ((g_left_encoder_edges + g_right_encoder_edges) != 0U) ? 1U : 0U;
}
...
static NOINLINE void encoder_reset_counts(void)
{
    ...
    g_bridge_encoder_error = 0;
    g_bridge_left_delta = 0;
    g_bridge_right_delta = 0;
    g_bridge_delta_error = 0;
    g_bridge_integral_error = 0;
    g_bridge_p_term = 0;
    g_bridge_i_term = 0;
    g_bridge_d_term = 0;
    g_bridge_target_correction = 0;
    g_bridge_encoder_correction = 0;
    g_bridge_left_command = PWM_LEFT_BASE_TICKS;
    g_bridge_right_command = PWM_RIGHT_BASE_TICKS;
    g_bridge_encoder_seen = 0U;
}
...
static NOINLINE void line_follow_update(void)
{
    ...
    if (sharp_turn) {
        g_line_correction = clamp_int32(
            ...
            LINE_SHARP_CORR_LIMIT);
        /* Positive error means the line is on the right: speed up left wheel. */
        set_pwm(
...
static NOINLINE void start_bridge_encoder_update(
    uint32_t left_base_ticks,
    uint32_t right_base_ticks)
{
    static int32_t previous_left_forward = 0;
    static int32_t previous_right_forward = 0;
    int32_t correction_delta;
    int32_t left_command;
    int32_t right_command;

    encoder_update_forward_totals();
    g_bridge_left_delta = g_left_forward_total - previous_left_forward;
    g_bridge_right_delta = g_right_forward_total - previous_right_forward;
    previous_left_forward = g_left_forward_total;
    previous_right_forward = g_right_forward_total;

    /* Positive error means the left wheel has travelled farther. */
    g_bridge_encoder_error = g_left_forward_total - g_right_forward_total;
    g_bridge_delta_error = g_bridge_left_delta - g_bridge_right_delta;
    g_bridge_integral_error = clamp_int32(
        g_bridge_integral_error + g_bridge_encoder_error,
        -BRIDGE_ENCODER_INTEGRAL_LIMIT,
        BRIDGE_ENCODER_INTEGRAL_LIMIT);

    g_bridge_p_term =
        g_bridge_encoder_error / BRIDGE_ENCODER_KP_DIVISOR;
    g_bridge_i_term =
        g_bridge_integral_error / BRIDGE_ENCODER_KI_DIVISOR;
    g_bridge_d_term =
        g_bridge_delta_error / BRIDGE_ENCODER_KD_DIVISOR;
    g_bridge_target_correction = clamp_int32(
        g_bridge_p_term + g_bridge_i_term + g_bridge_d_term,
        -BRIDGE_ENCODER_CORR_LIMIT,
        BRIDGE_ENCODER_CORR_LIMIT);

    correction_delta = clamp_int32(
        g_bridge_target_correction - g_bridge_encoder_correction,
        -BRIDGE_ENCODER_SLEW_LIMIT,
        BRIDGE_ENCODER_SLEW_LIMIT);
    g_bridge_encoder_correction += correction_delta;

    left_command =
        (int32_t) left_base_ticks - g_bridge_encoder_correction;
    right_command =
        (int32_t) right_base_ticks + g_bridge_encoder_correction;
    g_bridge_left_command = clamp_pwm(left_command);
    g_bridge_right_command = clamp_pwm(right_command);
    set_pwm(g_bridge_left_command, g_bridge_right_command);
}
    ...
    for (g_run_frame = 0U; g_run_frame < RUN_MAX_FRAMES; g_run_frame++) {
        ...
        if (g_phase == PHASE_START_BRIDGE) {
            ...
            if (g_start_bridge_frame_count <= START_BOOST_FRAMES) {
                g_start_boost_active = 1U;
                start_bridge_encoder_update(
                    START_BOOST_LEFT_TICKS,
                    START_BOOST_RIGHT_TICKS);
            } else {
                g_start_boost_active = 0U;
                start_bridge_encoder_update(
                    PWM_LEFT_BASE_TICKS,
                    PWM_RIGHT_BASE_TICKS);
            }
