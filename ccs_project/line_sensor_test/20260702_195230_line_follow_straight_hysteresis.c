/**
 * Combined line-following: straight stability + sharp turn capability.
 *
 * Strategy:
 *   Inherits v2's 5-mode state machine for turns (LOST_ACCEL, LINE_TURN)
 *   but fixes straight-line oscillation by adding exit hysteresis to
 *   STRAIGHT_FAST mode and retuning its control params closer to the
 *   simpler pre-sharp-turn baseline.
 *
 * Root cause of v2 straight-line jitter:
 *   center_seen requires: S4 ON AND no edge sensors AND |error|<=250.
 *   At high speed, minor perturbations trigger S1/S7 flicker ->
 *   center_seen drops -> g_center_stable_count resets -> exits
 *   STRAIGHT_FAST -> slower mode -> speed cycling creates more
 *   oscillation.  Rinse and repeat.
 *
 * Fixes:
 *   1. STRAIGHT_FAST exit hysteresis: 8-sample debounce before
 *      dropping out of straight fast mode.
 *   2. STRAIGHT_FAST control retuned toward pre-sharp-turn style:
 *      KP 34->22, LIMIT 90->120, DEADBAND 420->200, FILTER 5->8.
 *   3. CENTER_STABLE_SAMPLES 35->25 for faster straight entry.
 *   4. PWM slew step 60->80 for more responsive correction.
 *
 * Control modes:
 *   0 = straight fast (hysteresis-protected, simple P control)
 *   1 = slow on-line turn
 *   2 = full-lost accelerated recovery
 *   3 = center lock (aligning to center before straight fast)
 *   4 = medium align
 *
 * Sensors, from car front left to right:
 *   S1 DO -> PB5  S2 DO -> PB6  S3 DO -> PB7  S4 DO -> PA7
 *   S5 DO -> PA8  S6 DO -> PB9  S7 DO -> PB10
 *
 * Motor driver:
 *   PB0 -> AIN1  PB1 -> AIN2  PB2 -> BIN1  PB3 -> BIN2
 *   PB4 -> STBY  PA12 -> PWMA / TIMG0_CCP0  PA13 -> PWMB / TIMG0_CCP1
 *
 * Based on:
 *   20260702_191058_line_follow_center_first_v2_video_review.c  (turn logic)
 *   20260702_191813_restore_pre_sharp_turn_for_test.c  (straight-line tuning)
 */
...
/* ---- Straight fast (retuned toward pre-sharp-turn style) ---- */
#define PWM_LEFT_BASE_TICKS     715U
#define PWM_RIGHT_BASE_TICKS    682U
#define FAST_KP_DIVISOR         22
#define FAST_CORR_LIMIT         120
#define FAST_DEADBAND           200
#define FAST_FILTER_DIVISOR     8
#define FAST_EXIT_DEBOUNCE      8U

/* ---- Center lock (quick convergence) ---- */
#define CENTER_LEFT_BASE_TICKS  500U
#define CENTER_RIGHT_BASE_TICKS 475U
#define CENTER_KP_DIVISOR       40
#define CENTER_CORR_LIMIT       55
#define CENTER_DEADBAND         120
#define CENTER_FILTER_DIVISOR   6
#define CENTER_ERROR_LIMIT      300
#define CENTER_STABLE_SAMPLES   25U

/* ---- Line turn (kept from v2) ---- */
...
#define PWM_SLEW_STEP_TICKS     80U

/* ---- Lost accel (kept from v2) ---- */
...
volatile uint32_t g_fast_exit_count = 0;
...
static void update_line_position(void)
{
    ...
    int16_t new_error = (int16_t)(sum / (int32_t) count);
    g_line_last_error = g_line_error;

    if (new_error != 0) {
        g_last_turn_sign = (new_error > 0) ? 0x01 : (int8_t)(-1);
    } else if (g_line_error != 0) {
        g_last_turn_sign = (g_line_error > 0) ? 0x01 : (int8_t)(-1);
    }

    g_line_error = new_error;
}
...
static void line_follow_update(void)
{
    ...
    /*
     * Center stability tracking with STRAIGHT_FAST exit hysteresis.
     *
     * v2 problem: center_seen drops on transient edge flicker ->
     * g_center_stable_count resets -> exits STRAIGHT_FAST -> speed
     * cycling causes more oscillation.
     *
     * Fix: once in STRAIGHT_FAST (g_center_stable_count >= CENTER_STABLE_SAMPLES),
     * require FAST_EXIT_DEBOUNCE consecutive non-center samples before
     * actually dropping out.
     */
    if (center_seen) {
        g_fast_exit_count = 0U;
        if (g_center_stable_count < CENTER_STABLE_SAMPLES) {
            g_center_stable_count++;
        }
    } else if (g_center_stable_count >= CENTER_STABLE_SAMPLES) {
        /* Already in straight fast territory - use hysteresis */
        g_fast_exit_count++;
        if (g_fast_exit_count >= FAST_EXIT_DEBOUNCE) {
            g_center_stable_count = 0U;
            g_fast_exit_count = 0U;
        }
        /* Else: hold stable count, stay in straight fast */
    } else {
        g_center_stable_count = 0U;
    }

    /* ---- LOST ACCEL: fully lost line during a turn ---- */
    if (full_lost_accel) {
    ...
    /* ---- Mode selection for on-line cases ---- */
    ...
    if (online_turn) {
        ...
        /*
         * STRAIGHT_FAST: retuned toward pre-sharp-turn style.
         * Lighter filter, stronger KP, smaller deadband for
         * less drift and faster correction.  Hysteresis above
         * prevents spurious exit on edge sensor flicker.
         */
...
static void update_run_led(void)
{
    ...
    } else if (g_turn_mode == TURN_MODE_CENTER_LOCK) {
        period = LED_SETTLE_PERIOD_SAMPLES;
        on_samples = LED_SETTLE_ON_SAMPLES;
    } else if (g_turn_mode != TURN_MODE_STRAIGHT_FAST) {
