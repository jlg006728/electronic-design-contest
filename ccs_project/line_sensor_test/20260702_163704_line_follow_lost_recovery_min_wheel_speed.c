 ...
 *   20260702_163704: keep both wheels moving during lost-line recovery.
 *
...
#define PWM_LEFT_BASE_TICKS     650U
#define PWM_RIGHT_BASE_TICKS    620U
...
#define LOST_TURN_INNER_MIN_TICKS 220U
#define LOST_TURN_OUTER_MIN_TICKS 760U
...
static uint32_t clamp_pwm_with_min(int32_t ticks, uint32_t min_ticks)
{
    if (ticks < (int32_t) min_ticks) {
        return min_ticks;
    }
    if (ticks > (int32_t) PWM_MAX_TICKS) {
        return PWM_MAX_TICKS;
    ...
    if (sharp_turn) {
        ...
        if (lost_recovery) {
            ...
            if (g_line_correction >= 0) {
                set_pwm(
                    clamp_pwm_with_min(left_target, LOST_TURN_INNER_MIN_TICKS),
                    clamp_pwm_with_min(right_target, LOST_TURN_OUTER_MIN_TICKS));
            } else {
                set_pwm(
                    clamp_pwm_with_min(left_target, LOST_TURN_OUTER_MIN_TICKS),
                    clamp_pwm_with_min(right_target, LOST_TURN_INNER_MIN_TICKS));
            }
            return;
        } else {
