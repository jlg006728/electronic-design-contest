 ...
 *     LINE_KP_DIVISOR = 26
 *     LINE_CORR_LIMIT = 120
 *     LINE_NORMAL_DEADBAND = 350
 *   Sharp turn mode:
 *     LINE_SHARP_ERROR = 1800
 *     LINE_SHARP_KP_DIVISOR = 7
 *     LINE_SHARP_CORR_LIMIT = 430
 */
...
#define EXIT_LEFT_BASE_TICKS    600U
#define EXIT_RIGHT_BASE_TICKS   572U
#define PWM_MIN_TICKS           430U
#define PWM_MAX_TICKS           990U
#define PWM_SLEW_STEP_TICKS     90U
#define LINE_KP_DIVISOR         26
#define LINE_CORR_LIMIT         120
#define LINE_NORMAL_DEADBAND    350
#define LINE_NORMAL_FILTER_DIVISOR 4
#define EXIT_KP_DIVISOR         40
#define EXIT_CORR_LIMIT         80
#define EXIT_DEADBAND           250
#define EXIT_FILTER_DIVISOR     5
#define EXIT_SETTLE_SAMPLES     30U
#define LINE_SHARP_ERROR        1800
#define LINE_SHARP_KP_DIVISOR   7
#define LINE_SHARP_CORR_LIMIT   430
#define LINE_TURN_MEMORY_ERROR  600
#define LOST_TURN_BOOST_STEP_SAMPLES 5U
#define LOST_TURN_BOOST_TICKS   25
#define LOST_TURN_BOOST_MAX_TICKS 150
#define LINE_LOST_CORR_LIMIT    (LINE_SHARP_CORR_LIMIT + LOST_TURN_BOOST_MAX_TICKS)
...
volatile uint32_t g_exit_settle_count = 0;
...
static void line_follow_update(void)
{
    ...
    if ((g_line_valid == 0U) && (error == 0) && (g_last_turn_sign != 0)) {
        error = (int32_t) g_last_turn_sign * LINE_SHARP_ERROR;
    }

    bool lost_recovery = (g_line_valid == 0U) && (error != 0);
    bool sharp_turn = (abs_int32(error) >= LINE_SHARP_ERROR) || lost_recovery;

    if (lost_recovery) {
        g_turn_mode = 2U;
        g_exit_settle_count = 0U;
    } else if (sharp_turn) {
        g_turn_mode = 1U;
        g_exit_settle_count = 0U;
    } else {
        if (((previous_turn_mode == 1U) || (previous_turn_mode == 2U)) &&
            (g_line_valid != 0U)) {
            g_exit_settle_count = EXIT_SETTLE_SAMPLES;
            g_line_filtered_error = 0;
        }

        if (g_exit_settle_count > 0U) {
            g_turn_mode = 3U;
            g_exit_settle_count--;
        } else {
            g_turn_mode = 0U;
    ...
    bool exit_settle = (g_turn_mode == 3U);
    uint32_t left_base = exit_settle ? EXIT_LEFT_BASE_TICKS : PWM_LEFT_BASE_TICKS;
    uint32_t right_base = exit_settle ? EXIT_RIGHT_BASE_TICKS : PWM_RIGHT_BASE_TICKS;
    int32_t filter_divisor = exit_settle ? EXIT_FILTER_DIVISOR : LINE_NORMAL_FILTER_DIVISOR;
    int32_t deadband = exit_settle ? EXIT_DEADBAND : LINE_NORMAL_DEADBAND;
    int32_t kp_divisor = exit_settle ? EXIT_KP_DIVISOR : LINE_KP_DIVISOR;
    int32_t corr_limit = exit_settle ? EXIT_CORR_LIMIT : LINE_CORR_LIMIT;
