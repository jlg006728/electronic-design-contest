...
#define WIDE_LINE_ACTIVE_COUNT  4U
#define TURN_MEMORY_ERROR       250
#define WIDE_TURN_ERROR         2100
#define LOST_RECOVERY_ERROR     2000
#define LOST_TURN_MAX_SAMPLES   35U
...
volatile uint8_t g_wide_line_detected = 0;
volatile int8_t g_last_turn_sign = DEFAULT_TURN_SIGN;
volatile uint8_t g_turn_memory_valid = 1U;
...
static void update_line_position(void)
{
    ...
    if (count == 0U) {
        g_line_valid = 0U;
        g_line_lost_count++;
        g_wide_line_detected = 0U;
        if (g_line_lost_count <= LOST_TURN_MAX_SAMPLES) {
            g_line_error = g_line_last_error;
        } else {
            g_line_error = 0;
            g_line_last_error = 0;
            g_turn_memory_valid = 0U;
        }
        return;
    }
    ...
    g_wide_line_detected = (count >= WIDE_LINE_ACTIVE_COUNT) ? 1U : 0U;

    if (abs_int32((int32_t) g_line_error) >= TURN_MEMORY_ERROR) {
        g_line_last_error = g_line_error;
        g_last_turn_sign = (g_line_error > 0) ? 1 : -1;
        g_turn_memory_valid = 1U;
    }
}
...
static void line_follow_update(void)
{
    ...
    bool lost_recovery = (!line_visible) &&
        (g_has_seen_line != 0U) &&
        (g_line_last_error != 0) &&
        (g_line_lost_count <= LOST_TURN_MAX_SAMPLES);
    bool wide_turn = line_visible &&
        (g_wide_line_detected != 0U) &&
        (g_turn_memory_valid != 0U);

    if (wide_turn) {
        error = (int32_t) g_last_turn_sign * WIDE_TURN_ERROR;
    } else if (lost_recovery) {
        error = (int32_t) g_last_turn_sign * LOST_RECOVERY_ERROR;
    }
