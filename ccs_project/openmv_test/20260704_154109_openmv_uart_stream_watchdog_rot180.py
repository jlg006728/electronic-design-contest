# OpenMV H7 full self-test for the MSPM0 car project.
# 20260704_154109 version:
#   1. Rotate the image 180 degrees for the current upside-down installation.
#   2. Stream color frames on UART3 periodically without waiting for MSPM0
#      commands. This isolates one-way UART return wiring issues.
#   3. Add runtime exception recovery and periodic GC. If the script crashes,
#      blink red briefly and reset the OpenMV instead of silently stopping.
#
# Standalone test:
#   Run this script in OpenMV IDE. The IDE image window should refresh, the
#   terminal should print FPS/brightness/target information, and OpenMV LEDs
#   should blink.
#
# UART test with MSPM0:
#   OpenMV P4 / UART3_TX -> MSPM0 PA11 / UART0_RX
#   OpenMV P5 / UART3_RX <- MSPM0 PA10 / UART0_TX
#   OpenMV GND           -> MSPM0 GND
#
# UART commands from MSPM0:
#   'P' ping/status
#   'S' camera statistics
#   'C' red target detection
#
...
# The current OpenMV is mounted upside down. Enable both horizontal mirror
# and vertical flip so the IDE preview and reported cx/cy use the corrected
# upright coordinate system.
CAMERA_HMIRROR = True
CAMERA_VFLIP = True

# Default target: red marker/red laser spot under normal indoor light.
# Tune in OpenMV IDE if needed: Tools -> Machine Vision -> Threshold Editor.
RED_THRESHOLDS = [
    (20, 80, 20, 90, 10, 80),
]
...
def write_color_frame(detected, cx, cy, pixels, fps_x10, l_mean):
    flags = FLAG_CAMERA_OK
    if g_uart_rx_seen:
        flags |= FLAG_UART_RX_SEEN
    if detected:
        flags |= FLAG_TARGET_DETECTED
    uart.write(make_frame(TYPE_COLOR, flags, cx, cy, pixels, fps_x10, l_mean))
...
def send_reply(cmd):
    ...
    flags = FLAG_CAMERA_OK | FLAG_UART_RX_SEEN
    if detected:
        flags |= FLAG_TARGET_DETECTED
...
def standalone_tick():
    global g_last_print_ms, g_last_stream_ms, g_last_gc_ms
    detected, cx, cy, pixels, fps_x10, l_mean = capture_measure(True)
    update_leds(detected)

    now = millis()
    if now - g_last_stream_ms >= UART_STREAM_INTERVAL_MS:
        write_color_frame(detected, cx, cy, pixels, fps_x10, l_mean)
        g_last_stream_ms = now

    if now - g_last_print_ms >= 1000:
        print(
            "OMV stream fps=%.1f l=%d target=%d cx=%d cy=%d pixels=%d uart_seen=%d"
            % (
                fps_x10 / 10.0,
                l_mean,
    ...
    if now - g_last_gc_ms >= GC_INTERVAL_MS:
        gc.collect()
        g_last_gc_ms = now
...
def drain_uart_commands():
    count = 0
    while uart.any() and count < UART_DRAIN_LIMIT:
        cmd = uart.readchar()
        send_reply(cmd)
        count += 1
...
while True:
    try:
        ...
        try:
            print("OpenMV runtime error, rebooting:", err)
        except Exception:
            pass
        reboot_after_error()
