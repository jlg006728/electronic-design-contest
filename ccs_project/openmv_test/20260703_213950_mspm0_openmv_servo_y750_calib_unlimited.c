 ...
 *   20260703_213123_openmv_full_selftest_rot180.py
 *
 * Wiring:
...
#define SERVO_Y_HOME_US             750U
#define SERVO_STARTUP_HOME_FRAMES   75U
#define SERVO_X_MIN_US              900U
#define SERVO_X_MAX_US              2100U
#define SERVO_Y_MIN_US              750U
#define SERVO_Y_MAX_US              2100U
#define SERVO_Y_CALIB_MIN_US        0U
...
volatile uint8_t g_servo_calibrate_enable = 0;
...
static NOINLINE void clamp_home_position(void)
{
    ...
    /* During calibration, allow Y to go below the normal tracking limit. */
    g_servo_y_home_us = clamp_servo(
        g_servo_y_home_us, SERVO_Y_CALIB_MIN_US, SERVO_Y_MAX_US);
}
...
int main(void)
{
    ...
    while (1) {
        ...
        if (g_servo_calibrate_enable != 0U) {
            last_color_count = g_omv_color_frame_count;
        } else if (g_omv_color_frame_count != last_color_count) {
            last_color_count = g_omv_color_frame_count;
