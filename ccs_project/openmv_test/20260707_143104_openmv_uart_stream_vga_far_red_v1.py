...
# 20260707_143104:
#   Far-target red detection version.
#   1. Use VGA instead of QVGA so the distant target occupies more pixels.
#   2. Broaden red LAB thresholds, then use blob quality filters to reduce
#      false positives.
#   3. Reduce center penalty so an off-center distant target can still win.
#   4. MSPM0 must use IMG_WIDTH=640 and IMG_HEIGHT=480 with this script.
#
# 20260705_145746:
...
# Red target thresholds. The first range is the original conservative range.
# The second range is wider for distant/dimmer red targets.
RED_THRESHOLDS = [
    (20, 80, 20, 90, 10, 80),
    (12, 92, 14, 105, -5, 95),
]
...
def find_red_target(img):
    ...
    for blob in blobs:
        ...
        if blob.w() < MIN_RED_W or blob.h() < MIN_RED_H:
            continue

        try:
            density_percent = int(blob.density() * 100)
        except Exception:
            density_percent = 100

        if density_percent < MIN_RED_DENSITY_PERCENT:
            continue
...
if ENABLE_DEBUG_PRINT:
    print("OpenMV UART watchdog stream ready: UART3=115200, VGA RGB565, rot180=1")
