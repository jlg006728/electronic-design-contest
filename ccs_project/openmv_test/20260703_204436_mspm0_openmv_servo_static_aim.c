 ...
 *
 * Goal:
 *   Keep the car still, read red target coordinates from OpenMV, and move
 *   the PA14/PA17 gimbal so the target returns to image center.
 *
 * OpenMV script:
 *   C:/Users/1/Desktop/ELECTRIC/../电赛/ccs_project/openmv_test/
 *   20260703_162934_openmv_full_selftest.py
 *
 * Wiring:
...
#define AIM_DEAD_X_PIXELS           12
#define AIM_DEAD_Y_PIXELS           10
#define AIM_X_DIVISOR               18
#define AIM_Y_DIVISOR               18
#define AIM_MAX_STEP_US             8

#define SERVO_PERIOD_US             20000U
#define SERVO_X_CENTER_US           1500U
#define SERVO_Y_CENTER_US           1500U
#define SERVO_X_MIN_US              1250U
#define SERVO_X_MAX_US              1750U
#define SERVO_Y_MIN_US              1250U
#define SERVO_Y_MAX_US              1750U
...
{
    if (g_servo_x_us + SERVO_RECENTER_STEP_US < SERVO_X_CENTER_US) {
        g_servo_x_us += SERVO_RECENTER_STEP_US;
    } else if (g_servo_x_us > SERVO_X_CENTER_US + SERVO_RECENTER_STEP_US) {
        g_servo_x_us -= SERVO_RECENTER_STEP_US;
    } else {
        g_servo_x_us = SERVO_X_CENTER_US;
    }

    if (g_servo_y_us + SERVO_RECENTER_STEP_US < SERVO_Y_CENTER_US) {
        g_servo_y_us += SERVO_RECENTER_STEP_US;
    } else if (g_servo_y_us > SERVO_Y_CENTER_US + SERVO_RECENTER_STEP_US) {
        g_servo_y_us -= SERVO_RECENTER_STEP_US;
    } else {
        g_servo_y_us = SERVO_Y_CENTER_US;
    }
}
...
static NOINLINE void aiming_update_from_latest_color_frame(void)
{
    ...
    dx = (int16_t) (g_omv_cx - IMG_CENTER_X);
    dy = (int16_t) (g_omv_cy - IMG_CENTER_Y);
    g_aim_dx = dx;
    g_aim_dy = dy;
...
int main(void)
{
    ...
    while (1) {
        if ((g_servo_frame_count % OPENMV_REQUEST_INTERVAL_FRAMES) == 0U) {
