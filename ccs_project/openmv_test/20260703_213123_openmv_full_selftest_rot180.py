...
# 20260703_213123 version: camera image is rotated 180 degrees in software
# for the current upside-down OpenMV installation.
#
# Standalone test:
...
def standalone_tick():
    ...
    if now - g_last_print_ms >= 1000:
        print(
            "OMV selftest fps=%.1f l=%d target=%d cx=%d cy=%d pixels=%d"
            % (fps_x10 / 10.0, l_mean, 1 if detected else 0, cx, cy, pixels)
        )
        g_last_print_ms = now
...
print("OpenMV full self-test ready: UART3=115200, QVGA RGB565, rot180=1")
