/*
 * V60 MSPM0G3507 + MPU6050 isolated gyro test.
 *
 * Wiring:
 *   PA0 / I2C0 SDA -> MPU6050 SDA
 *   PA1 / I2C0 SCL -> MPU6050 SCL
 *   3.3V           -> MPU6050 VCC
 *   GND            -> MPU6050 GND and AD0
 *
 * LaunchPad jumpers:
 *   J4 OFF, J19 1:2, J20 1:2.
 *
 * Motors, servos, OpenMV UART and grayscale scanning are disabled.
 */
...
#define MPU_EXPECTED_WHO_AM_I               0x68U
...
{
    if (!mpu_read_registers(MPU_REG_WHO_AM_I,
            (uint8_t *) &g_mpu_who_am_i, 1U)) {
        g_mpu_error = 1U;
        return false;
    }
    if (g_mpu_who_am_i != MPU_EXPECTED_WHO_AM_I) {
        g_mpu_error = 2U;
        return false;
    }
...
int main(void)
{
    ...
    while (1) {
        if (mpu_read_gyro()) {
            ...
            /* +/-500 dps scale: 65.5 LSB per degree/second. */
