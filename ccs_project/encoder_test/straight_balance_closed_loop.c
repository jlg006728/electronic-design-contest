/**
 * Straight balance closed-loop test - MSPM0G3507 LaunchPad
 *
 * Purpose:
 *   Bench-test encoder feedback before putting the car on the ground.
 *   The wheels spin forward while the code adjusts left/right PWM so the
 *   encoder edge counts per 100 ms become close.
 *
 * Important calibration from current hardware:
 *   Left encoder count is negative when the left wheel moves forward.
 *   Right encoder count is positive when the right wheel moves forward.
 *
...
#define PWM_MIN_TICKS       620U
#define PWM_MAX_TICKS       1250U
#define PWM_CORR_LIMIT      220
...
static void balance_update(void)
{
     ...
     * Increase left PWM and decrease right PWM.
     */
    g_balance_error = g_right_delta_edges - g_left_delta_edges;
    g_pwm_correction = g_balance_error / 8;
    ...
    uint32_t left_pwm = clamp_pwm((int32_t) PWM_BASE_TICKS + g_pwm_correction);
    uint32_t right_pwm = clamp_pwm((int32_t) PWM_BASE_TICKS - g_pwm_correction);
