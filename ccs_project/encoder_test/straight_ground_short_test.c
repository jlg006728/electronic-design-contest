/**
 * Ground straight short test - MSPM0G3507 LaunchPad
 *
 * Purpose:
 *   First on-ground test after bench encoder balance.
 *   The car waits 3 seconds, drives forward at low speed for 1.5 seconds,
 *   then stops forever.
 *
 * Safety:
 *   Put the car on the ground with open space.
 *   Keep one hand near the power switch.
 *
 * Watch these CCS Expressions after it stops:
 *   g_run_left_edges
 *   g_run_right_edges
 *   g_run_error
 *   g_left_pwm_ticks
...
volatile uint8_t g_phase = 0;       /* 0 init, 1 waiting, 2 running, 3 stopped */
...
int main(void)
{
    ...
    g_phase = 1U;
    blink(6U, 150U, 350U);  /* 3 seconds warning before motion */
