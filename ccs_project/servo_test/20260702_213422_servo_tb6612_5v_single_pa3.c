/*
 * Single-servo movement test powered from the TB6612 board 5V/GND pins.
 *
 * Important:
 *   PA12/PA13 are already used by TB6612 PWMA/PWMB on the car.
 *   This version uses PA3 as the servo signal pin, matching the reserved
 *   gimbal servo pin plan.
 *
 *   The servo is NOT driven by TB6612 AOUT/BOUT motor outputs.
 *   The TB6612 board is used only as a temporary 5V power source.
 *   Servo control signal still comes from MSPM0 PA3.
 *
 * Wiring:
 *   TB6612 5V      -> servo red
 *   TB6612 GND     -> servo black/brown
 *   MSPM0 GND      -> TB6612 GND, common ground
 *   MSPM0 PA3      -> servo white/yellow/orange signal
 *
 * Safety:
 *   Only connect one unloaded servo first.
 *   Keep TB6612 STBY low, so the DC motors stay disabled.
 *   If servo red-black voltage drops below about 4.7V while moving,
 *   stop this test and use an independent 5V regulator instead.
 *
 * Watch, if USB debug is connected:
 *   g_step
 *   g_servo_us
 *   g_frame_count
 */
...
#define SERVO_PIN               DL_GPIO_PIN_3
#define UNUSED_SERVO_PIN        DL_GPIO_PIN_4
...
{
    init_output(IOMUX_PINCM8, SERVO_PORT, SERVO_PIN, false);
    init_output(IOMUX_PINCM9, SERVO_PORT, UNUSED_SERVO_PIN, false);
}
