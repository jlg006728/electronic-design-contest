/*
 * V93: full 2021 F medicine-car route without OpenMV.
 *
 * Wiring:
...
#define JUNCTION_CENTER_FORWARD_MS  220U
...
/* A-phase rising edge only. Recalibrate before final distance tuning. */
#define ENCODER_RISING_EDGES_PER_CM 736U
    ...
    const medicine_route_action_t *action)
{
    ...
    if (g_gray_changed_count >= ROUTE_EVENT_CONFIG.wide_active_min) {
        g_line_correction = 0;
        g_line_lost_count = 0U;
        set_pwm(PWM_LEFT_BASE_TICKS, PWM_RIGHT_BASE_TICKS);
        motors_forward();
...
static NOINLINE bool medicine_turn_step(bool mpu_sample_ok)
{
    ...
    if (g_turn_controller.phase == GYRO_TURN_PHASE_ROTATE) {
...
int main(void)
{
    ...
    if (g_gray_baseline_uniform == 0U) {
        fatal_error_loop(STOP_REASON_BASELINE_INVALID);
    }
    ...
    while (!g_mission.completed && !g_mission.fault) {
        const medicine_route_action_t *action;
