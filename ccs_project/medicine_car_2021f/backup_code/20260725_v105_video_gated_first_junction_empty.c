/*
 * V105: video-diagnosed first-junction gate and center-sensor confirmation.
 *
 * Wiring:
...
#define FIRST_JUNCTION_MIN_TRAVEL_FRAMES UINT16_MAX
#define FIRST_JUNCTION_ARM_DISTANCE_CM 50U
#define FIRST_JUNCTION_ARM_DISTANCE_EDGES \
    ((uint32_t)FIRST_JUNCTION_ARM_DISTANCE_CM * ENCODER_RISING_EDGES_PER_CM)
#define JUNCTION_TRIGGER_ACTIVE_MIN 4U
#define LINE_WIDE_ACTIVE_MIN        7U
#define JUNCTION_WIDE_CONFIRM_FRAMES 1U
#define FIRST_JUNCTION_WIDE_CONFIRM_FRAMES 3U
#define JUNCTION_CENTER_REQUIRED_MASK 0x18U
...
static NOINLINE void route_arm_first_junction_by_distance(void)
{
    if ((g_route_action_index != 0U) ||
        g_route_detector.armed ||
        (g_route_progress.average_forward_edges <
         FIRST_JUNCTION_ARM_DISTANCE_EDGES) ||
        (g_route_detector.normal_frames < 5U)) {
        return;
    }

    g_route_detector.armed = true;
}

static NOINLINE bool first_junction_center_sensor_gate_active(void)
{
    return (g_gray_changed_from_white_mask &
            JUNCTION_CENTER_REQUIRED_MASK) ==
           JUNCTION_CENTER_REQUIRED_MASK;
}
    ...
    const medicine_route_action_t *action)
{
    ...
    const bool line_centered =
        ...
        (g_gray_line_error <= ENDPOINT_CENTER_ERROR_MAX);
    uint8_t junction_active_count = g_gray_changed_count;
    route_event_t event;
    ...
    route_arm_first_junction_by_distance();
    ...
    if ((g_route_action_index == 0U) &&
        (destination == ROUTE_DESTINATION_JUNCTION) &&
        g_route_detector.armed &&
        !first_junction_center_sensor_gate_active()) {
        /* Ignore first-junction outer-only black-frame/edge hits after arming. */
        junction_active_count = 0U;
    }
