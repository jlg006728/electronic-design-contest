/*
 * V95: new MPU6050/MPU6500 independent validation firmware.
 *
 * Only I2C0 and the onboard LED are active. Motor direction, STBY, PWM,
 * servo and buzzer outputs are forced to safe levels for the entire test.
 *
 * Wiring:
 *   MPU VCC -> MSPM0 3.3 V
 *   MPU GND -> MSPM0 GND
 *   MPU SDA -> PA0 / I2C0 SDA
 *   MPU SCL -> PA1 / I2C0 SCL
...
static NOINLINE void delay_ms(uint32_t milliseconds)
{
    uint32_t index;
...
static NOINLINE bool mpu_read_gyro(void)
{
    ...
    g_gyro_x_raw =
        (int16_t)(((uint16_t)data[0] << 8) | (uint16_t)data[1]);
    g_gyro_y_raw =
...
{
    if (!mpu_read_registers(
            MPU_REG_WHO_AM_I,
            (uint8_t *)&g_mpu_who_am_i,
            1U)) {
        g_mpu_error = 1U;
        return false;
    }
...
static NOINLINE void fatal_error_loop(void)
{
    g_mpu_state = 4U;
    g_mpu_link_ok = 0U;
    while (1) {
...
int main(void)
{
    ...
    while (1) {
        /* Keep the safety state observable and impossible to clear in Watch. */
        g_motor_disabled = 1U;
        if (mpu_read_gyro()) {
            ...
            g_yaw_mdeg += (int32_t)(
                ((int64_t)g_gyro_z_mdps * MPU_SAMPLE_PERIOD_MS) / 1000);
