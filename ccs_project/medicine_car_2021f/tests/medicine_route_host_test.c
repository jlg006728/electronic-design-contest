...
static const route_progress_config_t UNIT_PROGRESS_CONFIG = {
...
static const route_progress_config_t PRODUCTION_PROGRESS_CONFIG = {
    .direction_tolerance_edges = 2U * ENCODER_RISING_EDGES_PER_CM,
    .no_motion_timeout_frames = 50U,
    .imbalance_check_edges = 5U * ENCODER_RISING_EDGES_PER_CM,
    .maximum_wheel_ratio = 4U,
...
static void test_encoder_profile_matches_50cm_calibration(void)
{
    assert(ENCODER_RISING_EDGES_PER_CM == 664U);
    assert((20U * ENCODER_RISING_EDGES_PER_CM) == 13280U);
    assert((70U * ENCODER_RISING_EDGES_PER_CM) == 46480U);
    assert((100U * ENCODER_RISING_EDGES_PER_CM) == 66400U);
    assert(PRODUCTION_PROGRESS_CONFIG.direction_tolerance_edges == 1328U);
    assert(PRODUCTION_PROGRESS_CONFIG.imbalance_check_edges == 3320U);
    assert(PRODUCTION_PROGRESS_CONFIG.endpoint_confirmation_grace_edges ==
           3320U);
}
...
static void test_encoder_faults_are_fail_closed(void)
{
    ...
    route_progress_begin(&progress, 0, 0, 1000U, 3000U);
    for (index = 1U;
         index < UNIT_PROGRESS_CONFIG.no_motion_timeout_frames;
         index++) {
        assert(route_progress_step(
                   &UNIT_PROGRESS_CONFIG,
                   &progress,
...
static void test_full_course_sensor_and_gyro_simulation(void)
{
    ...
    while (!mission.completed && !mission.fault && (guard < 10000U)) {
        ...
        if (active_action_index != mission.action_index) {
            ...
            if (medicine_route_is_follow_action(action->kind)) {
                const uint32_t simulated_edges_per_cm =
                    ENCODER_RISING_EDGES_PER_CM;
        ...
        if (medicine_route_is_follow_action(action->kind)) {
            ...
            if (follow_frame < 3U) {
                active_count = 8U;
            } else if (destination == ROUTE_DESTINATION_JUNCTION) {
                active_count =
                    (follow_frame <
                ...
                active_count = route_progress_endpoint_ready(&progress)
                    ? 0U
                    : 2U;
            }
...
int main(void)
{
    ...
    test_encoder_profile_matches_50cm_calibration();
    ...
    puts("medicine_route_host_test: 19/19 passed");
    return 0;
}
